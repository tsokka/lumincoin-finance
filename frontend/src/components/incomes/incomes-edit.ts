...
export class IncomesEdit {
    ...
    private async getCategory(id: string): Promise<void> {
        const category: CategoryResponse | null = await CategoriesService.getIncome(id);
    ...
    private async updateCategory(e: MouseEvent): Promise<void> {
        ...
        if (ValidationUtils.validateForm(this.validations)) {
            if (this.originalData && this.inputElement) {
                const result: HttpResult<CategoryResponse> = await HttpUtils.request<CategoryResponse>('/categories/income/' + String(this.originalData.id), 'PUT', true, {
