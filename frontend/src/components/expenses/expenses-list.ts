...
export class ExpensesList {
    ...
    private async deleteCategory(id: string): Promise<void> {
        const success: boolean = await CategoriesService.deleteExpense(Number(id), this.openNewRoute);
        if (success) {
    ...
    private async getExpenses(): Promise<void> {
        const result: HttpResult<Category[]> = await HttpUtils.request<Category[]>('/categories/expense');
        if (result.redirect) {
    ...
    private showRecords(expenses: Category[]): void {
        const recordsElement: HTMLElement | null = document.getElementById('records');
        if (recordsElement) {
            recordsElement.innerHTML = '';
        }

        expenses.forEach((expense: Category): void => {
